...
In den letzten beiden Tutorials hatten wir uns intensiv mit Streudiagrammen beschäftigt. In diesem Teil des Tutorials werden wir uns eine Reihe anderer Darstellungsformen anschauen, die `ggplot` für uns bereithält. Sie werden sehen, dass sich an der Grundlogik des Codes nicht viel ändert - sie müssen nur eben (ganz im Sinne der Grammar of Graphics) sozusagen neue "Vokabeln" lernen.
...
Wir hatten uns ja in den letzten beiden Teilen vor allem mit einem möglichen Zusammenhang zwischen den staatlichen Gesundheitsausgaben in einem Land und der Lebenserwartung beschäftigt. Bevor Zusammenhänge dieser Art zwischen zwei oder mehr Variablen untersucht werden, macht es in der Regel Sinn, sich die einzelnen Variablen einmal näher anzuschauen - und deshalb bleiben wir noch einen Moment bei der Variable der "Lebenserwartung". Schauen wir uns dazu einmal die Häufigkeitsverteilung der Lebenserwartung für die Staaten an, für die Daten vorliegen. Dazu bietet sich natürlich ein Histogramm an. Ein solches Histogramm zu erstellen ist mit `ggplot` relativ simpel. Sie geben wieder an, welchen Datensatz Sie verwenden wollen und übergeben im Mapping-Argument - also in der aes()-Funktion - nur eine Variable; in diesem Fall die Variable "wdi_lifexp" für die "Lebenserwartung". Danach lassen Sie sich mit der Funktion `geom_histogramm()` ein Histogramm ausgeben:
...
```{r}
p <- ggplot(qog, aes(x = continent, fill = as.factor(ht_colonial))) 
...
Natürlich lassen sich auch diese Darstellungen wieder kombinieren - häufig sieht man beispielsweise Kombinationen aus Histogrammen mit Linien aus einer Dichte-Schätzung. Wenn Sie allerdings Darstellungen kombinieren möchten, die eine unterschiedliche Achsenskalierung voraussetzen, dann müssen Sie dies separat berücksichtigen. 
...
Auch hier können wir Farbe (für die Linien) und Füllung (für den Körper der Dichtekurve) verwenden. Diese Figuren sehen oft recht hübsch aus. Aber wenn es mehrere gefüllte Bereiche auf dem Diagramm gibt, wie in diesem Fall, kann die Überlappung schwer zu lesen sein. (Die Abbildungen 4.18 und 4.19 sind Beispiele dafür.) Wenn Sie die Grundlinien der Dichtekurven verschwinden lassen möchten, können Sie stattdessen geom_line(stat = "density") verwenden. Damit entfällt auch die Möglichkeit, die ästhetische Füllung zu verwenden. Dies kann jedoch
in einigen Fällen eine Verbesserung sein. Probieren Sie es mit der Darstellung von Zustandsflächen aus und sehen Sie, wie sie sich vergleichen.

Genau wie bei geom_bar() werden die zählungsbasierten Standardwerte von den stat_-Funktionen, die von geom_histogram() und geom_ density() verwendeten stat_-Funktionen berechnet werden, proportionale Maße zurück, wenn wir sie danach fragen.
Für geom_density() kann die stat_density()-Funktion ihre Standard-Statistik ..density.. zurückgeben, oder ..scaled.., was eine proportionale Dichteschätzung liefert. Sie kann auch eine Statistik namens
...
p <- ggplot(qog, aes(x = continent, color = wdi_lifexp))
p + geom_bar()
...
g + geom_text(label = qog$ccodealp)
...
qog_sum <-
qog %>%
  select(wdi_dgovhexp, wdi_lifexp, wdi_pop, continent) %>%
  group_by(continent) %>%
...
qog_ts_sum <-
qog_ts %>%
  select(year, wdi_dgovhexp, wdi_lifexp, wdi_pop, ht_region, continent) %>%
